                ...
                <div class="w-full md:w-1/2 p-6 flex flex-col flex-grow overflow-hidden">
                    <div class="flex justify-between items-center mb-3 pb-4 border-b-2 sticky top-0 z-10">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                <a href="{{ route('otherProfile', ['id' => $post->user->id]) }}" class="font-medium text-sm hover:text-gray-500">{{$post->user->full_name}}</a>
                        ...
                        @php
                            $authUser = auth()->user();
                            $isFollowing = $authUser && isset($post->user) ? $authUser->isFollowing($post->user->id) : false;
                        @endphp

                        @if (!$authUser)
                            {{-- If the user is not logged in, show the Follow button --}}
                            <form method="POST" action="{{route('users.follow', $post->user->id)}}">
                                @csrf
                            ...
                            @if ($isFollowing)
                                <div class="relative group">
                                    ...
                                    <div class="absolute w-32 top-full left-0 rounded-lg p-1 mt-1 shadow-lg text-start scale-y-0 border-2 border-gray-200 group-hover:scale-y-100 origin-top duration-200 bg-white z-50">
                                        <form method="POST" action="{{route('users.unfollow', $post->user->id)}}">
                                            @csrf
                    ...
                    <div class="flex-grow overflow-auto scrollable-content pt-1">
                        <a href="{{ route('food.details', ['id' => $post->id]) }}" class="text-xl font-medium mb-1">{{$post->name}}</a>
        ...
        <div class="md:hidden bg-opacity-90 w-full max-w-sm p-4 flex flex-col">
            ...
            <div class="bg-white rounded-b-lg px-2">
                ...
                <div class="flex justify-between items-center mt-2">
                    <a href="{{route('food.details', $post->id)}}" class="text-base font-bold hover:text-gray-600">{{$post->name}}</a>
                    <div class="flex">
